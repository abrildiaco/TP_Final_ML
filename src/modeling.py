...
def build_tree_regression_pipeline(regressor):
    return Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("regressor", regressor),
    ])
...
def get_predictions(model, X, use_log_target=False):
    """
...
def evaluate_regression_model(model, X_train, y_train, X_val, y_val, use_log_target=False):
    ...
    Returns:
        pd.DataFrame: train and validation regression metrics
    """
    train_predictions = get_predictions(model, X_train, use_log_target)
    val_predictions = get_predictions(model, X_val, use_log_target)
...
def build_random_forest_model(n_estimators=300,max_depth=None,min_samples_split=2,min_samples_leaf=1,
    ...
    Random Forest is tree-based, so it does not require feature scaling.
    """
    ...
    return build_tree_regression_pipeline(regressor)
...
def evaluate_nn_predictions(y_true, predictions):
    return {
...
def train_neural_network(
    ...
    **model_params,
    ...
    model = build_neural_network_model(**model_params)

    target_train = np.log1p(y_train) if use_log_target else y_train
    model.fit(X_train, target_train)

    train_predictions = predict_nn(model, X_train, use_log_target)
    val_predictions = predict_nn(model, X_val, use_log_target)
